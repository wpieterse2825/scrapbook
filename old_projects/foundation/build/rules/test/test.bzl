load("//old_projects/foundation/build/rules:build.bzl", _level_3_operations = "level_3_operations")
load("//old_projects/foundation/build/rules/cpp:cpp_test.bzl", _cpp_test = "cpp_test")
load(
    "//old_projects/foundation/build/rules/test:test_source.bzl",
...
def test(
    ...
    _cpp_test(
        ...
        deps = [
            ":%s" % level_3_operation,
            "//old_projects/foundation/%s/%s/%s/test_fixtures:%s" % (
                level_1,
                level_2,
                level_3,
                level_3_operation,
