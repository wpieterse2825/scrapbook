...
load("//old_projects/idl_compiler:build_private.bzl", "IDLCompilerToolchainInfo")

_EXEC_GROUP_NAME="idl_compiler"
_EXEC_GROUP_TOOLCHAIN="//old_projects/idl_compiler/compiler:toolchain_type"
...
def _generate_attributes(item_type):
    return {
        ...
        "_use_persistent_workers": attr.label(
            default = Label("//old_projects/idl_compiler/compiler/%s:use_persistent_workers" % item_type),
            providers = [BuildSettingInfo],
        ),
        "_logging_level": attr.label(
            default = Label("//old_projects/idl_compiler/compiler/%s:logging_level" % item_type),
            providers = [BuildSettingInfo],
        ),
    }
