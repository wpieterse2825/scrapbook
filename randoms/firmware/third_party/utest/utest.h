...
#    define UTEST_STRNCMP(x, y, size)                                                                                                      \
        _Pragma("clang diagnostic push") _Pragma("clang diagnostic ignored \"-Wdisabled-macro-expansion\"") strncmp(x, y, size)            \
          _Pragma("clang diagnostic pop")
#else
...
#    define UTEST_EXPECT(x, y, cond)                                                                                                       \
        {                                                                                                                                  \
            _Pragma("clang diagnostic push") _Pragma("clang diagnostic ignored \"-Wlanguage-extension-token\"")                            \
              _Pragma("clang diagnostic ignored \"-Wc++98-compat-pedantic\"") _Pragma("clang diagnostic ignored \"-Wfloat-equal\"")        \
    ...
    UTEST_INITIALIZER(utest_register_##SET##_##NAME) {                                                                                     \
        ...
        utest_state.tests =                                                                                                                \
          UTEST_PTR_CAST(struct utest_test_state_s*,                                                                                       \
                         realloc(UTEST_PTR_CAST(void*, utest_state.tests), sizeof(struct utest_test_state_s) * utest_state.tests_length)); \
...
#define UTEST_F_TEARDOWN(FIXTURE) static void utest_f_teardown_##FIXTURE(int* utest_result, struct FIXTURE* utest_fixture)
...
#define UTEST_I_SETUP(FIXTURE) static void utest_i_setup_##FIXTURE(int* utest_result, struct FIXTURE* utest_fixture, size_t utest_index)
...
UTEST_WEAK int utest_main(int argc, const char* const argv[]) {
    ...
    for (index = 0; index < utest_state.tests_length; index++) {
        ...
        if (0 != result) {
            ...
            failed_testcases =
              UTEST_PTR_CAST(size_t*, realloc(UTEST_PTR_CAST(void*, failed_testcases), sizeof(size_t) * failed_testcases_length));
            failed_testcases[failed_testcase_index] = index;
            failed++;
            printf("%s[  FAILED  ]%s %s (%" UTEST_PRId64 "ns)\n", colours[RED], colours[RESET], utest_state.tests[index].name, ns);
        } else {
            printf("%s[       OK ]%s %s (%" UTEST_PRId64 "ns)\n", colours[GREEN], colours[RESET], utest_state.tests[index].name, ns);
        }
    }

    printf("%s[==========]%s %" UTEST_PRIu64 " test cases ran.\n", colours[GREEN], colours[RESET], ran_tests);
    printf("%s[  PASSED  ]%s %" UTEST_PRIu64 " tests.\n", colours[GREEN], colours[RESET], ran_tests - failed);
