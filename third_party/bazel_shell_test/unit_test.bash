...
#### Configuration variables (may be overridden by test_environment.bash or the suite):

# This function may be called by test_environment.bash or a test suite to enable errexit
...
source "${DIR}/unit_test_utilities.bash" || { echo "unit_test_utilities.bash not found" >&2; exit 1; }
...
# Called prior to set_up. For use by test_environment.bash.
function testenv_set_up() {
    :
}

# Usage: testenv_tear_down
# Called after tear_down. For use by test_environment.bash.
