...
int app_setup_uring(struct submitter* s) {
     ...
     * easy reference */
    sring->head         = (unsigned int*)(((uint8_t*)sq_ptr) + p.sq_off.head);
    sring->tail         = (unsigned int*)(((uint8_t*)sq_ptr) + p.sq_off.tail);
    sring->ring_mask    = (unsigned int*)(((uint8_t*)sq_ptr) + p.sq_off.ring_mask);
    sring->ring_entries = (unsigned int*)(((uint8_t*)sq_ptr) + p.sq_off.ring_entries);
    sring->flags        = (unsigned int*)(((uint8_t*)sq_ptr) + p.sq_off.flags);
    sring->array        = (unsigned int*)(((uint8_t*)sq_ptr) + p.sq_off.array);

    /* Map in the submission queue entries array */
    s->sqes =
      (io_uring_sqe*)mmap(0, p.sq_entries * sizeof(struct io_uring_sqe), PROT_READ | PROT_WRITE, MAP_SHARED | MAP_POPULATE, s->ring_fd, IORING_OFF_SQES);
     ...
     * easy reference */
    cring->head         = (unsigned int*)(((uint8_t*)cq_ptr) + p.cq_off.head);
    cring->tail         = (unsigned int*)(((uint8_t*)cq_ptr) + p.cq_off.tail);
    cring->ring_mask    = (unsigned int*)(((uint8_t*)cq_ptr) + p.cq_off.ring_mask);
    cring->ring_entries = (unsigned int*)(((uint8_t*)cq_ptr) + p.cq_off.ring_entries);
    cring->cqes         = (io_uring_cqe*)(((uint8_t*)cq_ptr) + p.cq_off.cqes);
