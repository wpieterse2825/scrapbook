...
bool VideoVulkan_CreateExtensions(void) {
    if (SDL_Vulkan_GetInstanceExtensions(vulkan_main_window, &vulkan_extension_count, NULL) == 0) {
        vulkan_common_export->log->Print(PRINT_LEVEL_WARNING, "Failed to query Vulkan for extension count: %s\n", SDL_GetError());

        return false;
    }

    size_t allocation_count = sizeof(const char*) * vulkan_extension_count;
    vulkan_extensions       = (const char**)vulkan_common_export->memory_system->Allocate(allocation_count, MEMORY_TAG_RENDERER);
    if (vulkan_extensions == NULL) {
        vulkan_common_export->log->Print(PRINT_LEVEL_WARNING, "Failed to allocate memory to store Vulkan extension information.");

        return false;
    }

    if (SDL_Vulkan_GetInstanceExtensions(vulkan_main_window, &vulkan_extension_count, vulkan_extensions) == 0) {
        vulkan_common_export->log->Print(PRINT_LEVEL_WARNING, "Failed to query Vulkan for extension information: %s\n", SDL_GetError());

        return false;
    }

    if (vulkan_extension_count == 0) {
        vulkan_common_export->log->Print(PRINT_LEVEL_INFORMATION, "No registered Vulkan extensions.\n");
    } else {
        vulkan_common_export->log->Print(PRINT_LEVEL_INFORMATION, "Vulkan extensions (%ld):\n", vulkan_extension_count);
        ...
        for (int32_t extension_index = 0; extension_index < vulkan_extension_count; extension_index++) {
            ...
            vulkan_common_export->log->Print(PRINT_LEVEL_INFORMATION, " - %s\n", current_extension);
        }
    }
...
bool VideoVulkan_CreateLayers(void) {
    ...
    if (vulkan_result != VK_SUCCESS) {
        vulkan_common_export->log->Print(PRINT_LEVEL_WARNING, "Failed to query Vulkan for layer count.");

        return false;
    }

    size_t allocation_count = sizeof(VkLayerProperties) * vulkan_layer_count;
    vulkan_layers           = (VkLayerProperties*)vulkan_common_export->memory_system->Allocate(allocation_count, MEMORY_TAG_RENDERER);
    if (vulkan_layers == NULL) {
        vulkan_common_export->log->Print(PRINT_LEVEL_WARNING, "Failed to allocate memory to store Vulkan layer information.");
    ...
    if (vulkan_result != VK_SUCCESS) {
        vulkan_common_export->log->Print(PRINT_LEVEL_WARNING, "Failed to query Vulkan for layer information.");

        return false;
    }

    if (vulkan_layer_count == 0) {
        vulkan_common_export->log->Print(PRINT_LEVEL_INFORMATION, "No registered Vulkan layers.\n");
    } else {
        vulkan_common_export->log->Print(PRINT_LEVEL_INFORMATION, "Vulkan layers (%ld):\n", vulkan_layer_count);
        ...
        for (int32_t layer_index = 0; layer_index < vulkan_layer_count; layer_index++) {
            ...
            vulkan_common_export->log->Print(PRINT_LEVEL_INFORMATION,
                                             " - %s (%ld - %ld) - %s\n",
...
bool VideoVulkan_CreateUseExtensions(void) {
    ...
    vulkan_use_extensions = (const char**)vulkan_common_export->memory_system->Allocate(allocation_count, MEMORY_TAG_RENDERER);
    if (vulkan_use_extensions == NULL) {
        vulkan_common_export->log->Print(PRINT_LEVEL_WARNING, "Failed to allocate memory for the enable Vulkan extensions array.");

        return false;
    }

    vulkan_common_export->log->Print(PRINT_LEVEL_INFORMATION, "Using the following Vulkan extension:\n");
    ...
    vulkan_common_export->log->Print(PRINT_LEVEL_INFORMATION, "Enabled %ld Vulkan extensions.\n", vulkan_use_extension_count);
...
bool VideoVulkan_CreateUseLayers(void) {
    ...
    vulkan_use_layers = (const char**)vulkan_common_export->memory_system->Allocate(allocation_count, MEMORY_TAG_RENDERER);
    if (vulkan_use_layers == NULL) {
        vulkan_common_export->log->Print(PRINT_LEVEL_WARNING, "Failed to allocate memory for the enable Vulkan layers array.");

        return false;
    }

    int64_t debug_enable = vulkan_common_export->variable->GetInteger(vulkan_enable_debugging_variable);
    ...
    if (debug_enable == 1) {
        if (can_use_debugging == false) {
            vulkan_common_export->log->Print(PRINT_LEVEL_WARNING,
                                             "Requested to use Vulkan debug layer, but the layer could not be found.\n");
        }
    }

    vulkan_common_export->log->Print(PRINT_LEVEL_INFORMATION, "Using the following Vulkan layers:\n");
    ...
    vulkan_common_export->log->Print(PRINT_LEVEL_INFORMATION, "Enabled %ld Vulkan layers.\n", vulkan_use_layer_count);
...
bool VideoVulkan_CreateInstance(void) {
    ...
    if (vulkan_result != VK_SUCCESS) {
        if (vulkan_result == VK_ERROR_INCOMPATIBLE_DRIVER) {
            vulkan_common_export->log->Print(PRINT_LEVEL_WARNING, "Failed to find a suitable Vulkan driver.");

            return false;
        } else {
            vulkan_common_export->log->Print(PRINT_LEVEL_WARNING, "Failed to create the Vulkan instance object.");
...
bool VideoVulkan_CreateDebugCallback(void) {
    if (can_use_debugging == true) {
        int64_t debug_enable          = vulkan_common_export->variable->GetInteger(vulkan_enable_debugging_variable);
        int64_t debug_use_trace       = vulkan_common_export->variable->GetInteger(vulkan_debug_trace_variable);
        int64_t debug_use_information = vulkan_common_export->variable->GetInteger(vulkan_debug_information_variable);
        int64_t debug_use_warning     = vulkan_common_export->variable->GetInteger(vulkan_debug_warning_variable);
        int64_t debug_use_performance = vulkan_common_export->variable->GetInteger(vulkan_debug_performance_warning_variable);

        if (debug_enable == 1) {
            VkDebugReportFlagsEXT debug_enable_flags = 0;

            vulkan_common_export->log->Print(PRINT_LEVEL_INFORMATION, "Enabling Vulkan debug layer with the following bits:\n");
            if (debug_use_trace == 1) {
                debug_enable_flags |= VK_DEBUG_REPORT_DEBUG_BIT_EXT;

                vulkan_common_export->log->Print(PRINT_LEVEL_INFORMATION, " - Trace\n");
            }

            if (debug_use_information == 1) {
                debug_enable_flags |= VK_DEBUG_REPORT_INFORMATION_BIT_EXT;

                vulkan_common_export->log->Print(PRINT_LEVEL_INFORMATION, " - Information\n");
            }

            if (debug_use_warning == 1) {
                debug_enable_flags |= VK_DEBUG_REPORT_WARNING_BIT_EXT;

                vulkan_common_export->log->Print(PRINT_LEVEL_INFORMATION, " - Warning\n");
            }

            if (debug_use_performance == 1) {
                debug_enable_flags |= VK_DEBUG_REPORT_PERFORMANCE_WARNING_BIT_EXT;

                vulkan_common_export->log->Print(PRINT_LEVEL_INFORMATION, " - Performace\n");
            }

            // Always enable error logging when using the debug interface.
            debug_enable_flags |= VK_DEBUG_REPORT_ERROR_BIT_EXT;

            vulkan_common_export->log->Print(PRINT_LEVEL_INFORMATION, " - Error\n");
            ...
            if (vulkan_result != VK_SUCCESS) {
                vulkan_common_export->log->Print(PRINT_LEVEL_WARNING, "Failed to enable debug logging for Vulkan.\n");
            }
        }
    }
...
bool VideoVulkan_CreatePhysicalDeviceBuffers(void) {
    ...
    if (vulkan_physical_device_count == 0) {
        vulkan_common_export->log->Print(PRINT_LEVEL_WARNING, "Failed to find any Vulkan devices.");

        return false;
    }

    vulkan_physical_devices = (VkPhysicalDevice*)vulkan_common_export->memory_system->Allocate(
      sizeof(VkPhysicalDevice) * vulkan_physical_device_count, MEMORY_TAG_RENDERER);
    if (vulkan_physical_devices == NULL) {
        vulkan_common_export->log->Print(PRINT_LEVEL_WARNING, "Failed to allocate memory to store the physical Vulkan devices.");

        return false;
    }

    vulkan_physical_device_properties = (VkPhysicalDeviceProperties*)vulkan_common_export->memory_system->Allocate(
    ...
    if (vulkan_physical_device_properties == NULL) {
        vulkan_common_export->log->Print(PRINT_LEVEL_WARNING, "Failed to allocate memory to store the physical Vulkan devices properties.");

        return false;
    }

    vulkan_physical_device_queue_properties_counts =
      (uint32_t*)vulkan_common_export->memory_system->Allocate(sizeof(uint32_t) * vulkan_physical_device_count, MEMORY_TAG_RENDERER);
    ...
    vulkan_physical_device_queue_properties = (VkQueueFamilyProperties**)vulkan_common_export->memory_system->Allocate(
    ...
    if (vulkan_physical_device_queue_properties == NULL) {
        vulkan_common_export->log->Print(PRINT_LEVEL_WARNING,
                                         "Failed to allocate memory to store the physical Vulkan devices queue properties.");
    ...
    for (size_t device_index = 0; device_index < vulkan_physical_device_count; device_index++) {
        ...
        vulkan_physical_device_queue_properties[device_index] = (VkQueueFamilyProperties*)vulkan_common_export->memory_system->Allocate(
    ...
    vulkan_common_export->log->Print(PRINT_LEVEL_INFORMATION, "Vulkan devices (%ld):\n", vulkan_physical_device_count);
    for (size_t device_index = 0; device_index < vulkan_physical_device_count; device_index++) {
        VkPhysicalDeviceProperties* current_physical_device_properties = &vulkan_physical_device_properties[device_index];
        ...
        switch (current_physical_device_properties->vendorID) {
            case 0x8086:
                vulkan_common_export->log->Print(PRINT_LEVEL_INFORMATION, "   - Vendor         : Intel\n");
                break;
            case 0x10DE:
                vulkan_common_export->log->Print(PRINT_LEVEL_INFORMATION, "   - Vendor         : nVidia\n");
                break;
            case 0x1002:
                vulkan_common_export->log->Print(PRINT_LEVEL_INFORMATION, "   - Vendor         : AMD\n");
                break;
            default:
                vulkan_common_export->log->Print(
                  PRINT_LEVEL_INFORMATION, "   - Vendor         : Unknown (%ld)\n", current_physical_device_properties->vendorID);
                break;
        }

        vulkan_common_export->log->Print(
          PRINT_LEVEL_INFORMATION, "   - Device  ID     : %ld\n", current_physical_device_properties->deviceID);
        vulkan_common_export->log->Print(
          PRINT_LEVEL_INFORMATION, "   - Driver Version : %ld\n", current_physical_device_properties->driverVersion);
        ...
        switch (current_physical_device_properties->deviceType) {
            case VK_PHYSICAL_DEVICE_TYPE_OTHER:
                vulkan_common_export->log->Print(PRINT_LEVEL_INFORMATION, "   - Vulkan Type    : Other\n");
                break;
            case VK_PHYSICAL_DEVICE_TYPE_INTEGRATED_GPU:
                vulkan_common_export->log->Print(PRINT_LEVEL_INFORMATION, "   - Vulkan Type    : Integrated GPU\n");
                break;
            case VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU:
                vulkan_common_export->log->Print(PRINT_LEVEL_INFORMATION, "   - Vulkan Type    : Discrete GPU\n");
                break;
            case VK_PHYSICAL_DEVICE_TYPE_VIRTUAL_GPU:
                vulkan_common_export->log->Print(PRINT_LEVEL_INFORMATION, "   - Vulkan Type    : Virtual GPU\n");
                break;
            case VK_PHYSICAL_DEVICE_TYPE_CPU:
                vulkan_common_export->log->Print(PRINT_LEVEL_INFORMATION, "   - Vulkan Type    : CPU\n");
                break;
            default:
                vulkan_common_export->log->Print(PRINT_LEVEL_INFORMATION, "   - Vulkan Type    : Unknown\n");
                break;
        }
    }
...
bool VideoVulkan_CreatePhysicalDevice(void) {
    ...
    if (vulkan_physical_device_transfer_queue == -1) {
        vulkan_common_export->log->Print(PRINT_LEVEL_WARNING, "Failed to find a Vulkan transfer queue.");
    ...
    if (vulkan_physical_device_compute_queue == -1) {
        vulkan_common_export->log->Print(PRINT_LEVEL_WARNING, "Failed to find a Vulkan compute queue.");
    ...
    if (vulkan_physical_device_graphics_queue == -1) {
        vulkan_common_export->log->Print(PRINT_LEVEL_WARNING, "Failed to find a Vulkan graphics queue.");
...
bool VideoVulkan_DestroyPhysicalDeviceBuffers(void) {
    if (vulkan_physical_device_queue_properties != NULL) {
        for (uint32_t device_index = 0; device_index < vulkan_physical_device_count; device_index++) {
            vulkan_common_export->memory_system->Free(vulkan_physical_device_queue_properties[device_index]);

            vulkan_physical_device_queue_properties[device_index] = NULL;
        }

        vulkan_common_export->memory_system->Free(vulkan_physical_device_queue_properties);

        vulkan_physical_device_queue_properties = NULL;
    }

    if (vulkan_physical_device_queue_properties_counts != NULL) {
        vulkan_common_export->memory_system->Free(vulkan_physical_device_queue_properties_counts);

        vulkan_physical_device_queue_properties_counts = NULL;
    }

    if (vulkan_physical_device_properties != NULL) {
        vulkan_common_export->memory_system->Free(vulkan_physical_device_properties);

        vulkan_physical_device_properties = NULL;
    }

    if (vulkan_physical_devices != NULL) {
        vulkan_common_export->memory_system->Free(vulkan_physical_devices);
...
bool VideoVulkan_DestroyUseLayers(void) {
    if (vulkan_use_layers != NULL) {
        vulkan_common_export->memory_system->Free(vulkan_use_layers);
...
bool VideoVulkan_DestroyUseExtensions(void) {
    if (vulkan_use_extensions != NULL) {
        vulkan_common_export->memory_system->Free(vulkan_use_extensions);
...
bool VideoVulkan_DestroyLayers(void) {
    if (vulkan_layers != NULL) {
        vulkan_common_export->memory_system->Free(vulkan_layers);
        vulkan_layers = NULL;
    }
...
bool VideoVulkan_DestroyExtensions(void) {
    if (vulkan_extensions != NULL) {
        vulkan_common_export->memory_system->Free(vulkan_extensions);
...
static VKAPI_ATTR VkBool32 VKAPI_CALL VideoVulkan_DebugCallback(VkDebugReportFlagsEXT      flags,
    ...
    vulkan_common_export->string->Print(message_buffer,
    ...
    if (flags & VK_DEBUG_REPORT_DEBUG_BIT_EXT) {
        vulkan_common_export->log->Print(PRINT_LEVEL_DEVELOPER, message_buffer);
    } else if (flags & VK_DEBUG_REPORT_INFORMATION_BIT_EXT) {
        vulkan_common_export->log->Print(PRINT_LEVEL_INFORMATION, message_buffer);
    } else if (flags & VK_DEBUG_REPORT_WARNING_BIT_EXT) {
        vulkan_common_export->log->Print(PRINT_LEVEL_WARNING, message_buffer);
    } else if (flags & VK_DEBUG_REPORT_PERFORMANCE_WARNING_BIT_EXT) {
        vulkan_common_export->log->Print(PRINT_LEVEL_WARNING, message_buffer);
    } else if (flags & VK_DEBUG_REPORT_ERROR_BIT_EXT) {
        vulkan_common_export->log->Print(PRINT_LEVEL_ERROR, message_buffer);
    }
