...
_EXEC_GROUP_TOOLCHAIN = "//idl_compiler/generator:toolchain_type"
...
def _generate_attributes(language_type):
    return {
        ...
        "_use_persistent_workers": attr.label(
            default = Label("//idl_compiler/generator/%s:use_persistent_workers" % language_type),
            providers = [BuildSettingInfo],
        ),
        "_logging_level": attr.label(
            default = Label("//idl_compiler/generator/%s:logging_level" % language_type),
            providers = [BuildSettingInfo],
        ),
    }
